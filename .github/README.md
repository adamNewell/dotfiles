...
<p align="center"><i>My dotfiles for configuring literally everything on a new machine</i></p>
<p align="center">
  <a href="https://github.com/adamNewell/dotfiles" title="Automate all the things!">
    <img width="140" src="https://github.com/adamNewell/dotfiles/raw/master/.github/logo.png" />
...
One of the beautiful things about Linux (and to a lesser extent MacOS) is how easily customizable everything is. Usually these custom configurations are stored in files that start with a dot (hence dotfiles) and are typically located in your users home `~`, or better yet `~/.config` (even this can be customized, for apps that respect the XDG Base Directory spec). Some examples of dotfiles that you're likely already familiar with include `.gitconfig`, `.zshrc` or `.vimrc`.
...
| `XDG_STATE_HOME`  | `~/.local/var/state` |
