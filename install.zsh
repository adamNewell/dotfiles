...
init_submodules() {
    ...
    git submodule update || die 'Failed to update git submodules'
    debug 3 "Submodule status:"
...
install_dotfiles() {
    ...
    # First stow home directory contents
    if STOW_DIR=$DOTFILES stow -v -t $HOME home; then
        debug 3 "Successfully stowed home directory contents"
    else
        debug 3 "Failed to stow home directory"
        die "Failed to stow home directory contents"
    fi

    # Create .config and .local directories if they don't exist
    mkdir -p "$HOME/.config" "$HOME/.local"

    # Then stow .config directory contents into .config
    if STOW_DIR=$DOTFILES/.config stow -v -t $HOME/.config .; then
    ...
    # Then stow .local directory contents into .local
    if STOW_DIR=$DOTFILES/.local stow -v -t $HOME/.local .; then
        ...
        if (( DEBUG_LEVEL >= 3 )); then
            debug 3 "New symlinks created:"
            find "$HOME/.config" "$HOME/.local" -type l -exec ls -la {} \; 2>/dev/null | grep "$DOTFILES"
        fi
    else
